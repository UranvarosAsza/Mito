<template>
  <div class="SumPanel">
    <div class="sumCard">
      <div class="sumHeader">Thanks for buying your tickets at mito airlines</div>
      <div class="flights"></div>
      <div class="total">
        <div class="totalValue">Total: {{ total }}</div>
        <RouterLink class="endme" :to="{ name: 'HomePage' }" @click="resetData">
          No, thanks (reset)</RouterLink
        >
        <RouterView />
      </div>
    </div>
...
export default {
  ...
  methods: {
    resetData() {
      ...
      localStorage.setItem('selectedReturnTicket', '')
    },
    hidePanel() {
